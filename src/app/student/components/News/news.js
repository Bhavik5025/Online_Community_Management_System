...
const News = () => {
  ...
  return (
    <>
      {loading && <Loading />} {/* Show a loading indicator */}
          ...
          {data.map((article, index) => (
            <div key={index} className="bg-white hover:scale-105 transition-transform duration-500 shadow-lg mt-5 h-auto rounded-xl w-2/3 mx-auto">
              ...
              <p className="font-poppins ml-3">
                <i className="fa fa-file-text mr-4" aria-hidden="true"></i>
                {article.description}
              </p>
              <p className="font-poppins ml-3">
                <i className="fa fa-calendar mr-4" aria-hidden="true"></i>
                Published on: {article.publishedAt}
              </p>
              <div className="mb-5 ml-3 mt-3">
                <a href={article.url} className="font-semibold text-cyan-500 border-b-2 border-cyan-500" target="_blank" rel="noopener noreferrer">
                  Read More
                </a>
              </div>
              <div className="h-3"></div>
